...
def manual_test(mock_time_str: str):
    ...
    for ticker, data in signals.items():
        ...
        if error:
            ...
            if data.get("ma") is not None:
                indicators.append(f"{data.get('ma_label', 'MA')}:{data['ma']:.4f}")
            elif data.get("market_ma") is not None:
                indicators.append(f"MARKET_MA:{data['market_ma']:.4f}")
            ...
            if "legacy_ema" in data:
                indicators.append(f"LEGACY_EMA:{data['legacy_ema']:.4f}")
            if "fast_ema" in data:
                indicators.append(f"FAST_EMA:{data['fast_ema']:.4f}")
            if "high20" in data:
                indicators.append(f"BREAKOUT_WINDOW:{data['high20']:.4f}")
            if "qqq_above_market_ma" in data:
                indicators.append(f"QQQ>MARKET_MA:{'是' if data['qqq_above_market_ma'] else '否'}")
            if "legacy_ema_above_fast_ema" in data:
                indicators.append(f"LEGACY_EMA>FAST_EMA:{'是' if data['legacy_ema_above_fast_ema'] else '否'}")
