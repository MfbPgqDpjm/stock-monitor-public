...
def build_holding_monitor_rows(
    ...
    momentum_configs = _load_momentum_config_by_ticker(config)
    market_rows = _build_market_holding_rows(us_configs, signals, realtime_quotes, config=config)
...
def _build_market_holding_rows(
    ...
    for mc in us_configs:
        ...
        sell_reason = _format_market_sell_reason(signal.get("sell_reason"), benchmark)
        status = f"建议卖出：{sell_reason}" if signal.get("signal") == "卖出" and sell_reason else "建议卖出" if signal.get("signal") == "卖出" else "持有"
        ...
        row = {
            ...
            "持仓最大回撤": _market_strategy_drawdown(signal, metrics),
            "指标": indicator,
            "指标线": indicator_line,
            "持股天数": hold_days,
...
def _build_momentum_holding_rows(
    ...
    for pos in positions:
        ...
        risk = _risk_fields(
            ...
            config=config,
            signal_ticker=indicator_info.get("signal_ticker"),
        )
        sell_reason = str(pos.get("sell_reason", "") or "")
        status = f"建议卖出：{sell_reason}" if pos.get("action_plan") == "待卖出" and sell_reason else "建议卖出" if pos.get("action_plan") == "待卖出" else "持有"
...
def _load_momentum_config_by_ticker(config: dict) -> Dict[str, Dict[str, Any]]:
    return {
        str(row.get("ticker", "")).upper(): row
        for row in get_momentum_ticker_configs(config or {})
        if row.get("ticker")
    }
...
def _momentum_indicator_info(
    ...
    indicator = "FAST_EMA/SLOW_EMA"
    ema_window_i = 100

    line = None
    if line is None and str(position.get("signal_ticker") or ticker_u).strip().upper() == signal_ticker:
        line = position.get("trend_ema", position.get("slow_ema", position.get("fast_ema")))
    if line is None:
...
def _risk_fields(
    ...
    return {
        ...
        "风险分": _int_or_none(risk.get("score")) or 0,
        "风险触发": risk.get("reason") or "未触发",
        "价格/MARKET_MA": _to_float(risk.get("price_market_ma_deviation_pct")),
        "价格/FAST_EMA": _to_float(risk.get("price_fast_ema_deviation_pct")),
    }
...
def _market_indicator(signal: dict, strategy_state: dict) -> tuple[str, Any]:
    benchmark = str(signal.get("benchmark") or strategy_state.get("benchmark") or "").strip()
    threshold = signal.get("exit_threshold", signal.get("threshold"))
    return benchmark, _to_float(threshold)


def _format_market_sell_reason(reason: object, benchmark: str) -> str:
    raw = str(reason or "").strip()
    if raw == "drawdown":
        bm = benchmark or "观测基准"
...
def _market_strategy_drawdown(signal: dict, metrics: dict) -> Any:
    drawdown = signal.get("drawdown") if isinstance(signal, dict) else None
    if isinstance(drawdown, (int, float)):
        return -abs(float(drawdown) * 100.0)
