...
def _high_series(data: Optional[pd.DataFrame]) -> Optional[pd.Series]:
    if data is None or data.empty:
        return None
    source = "High" if "High" in data.columns else "Close" if "Close" in data.columns else None
    if source is None:
        return None
    high = data[source]
...
def _ytd_return_from_data(data: Optional[pd.DataFrame], now_et: datetime) -> Optional[Dict[str, Any]]:
    ...
    ytd_high = None
    ytd_high_drawdown_pct = None
    if high is not None:
        in_year_high = high[(high.index >= jan1) & (high.index <= latest_date)]
        if not in_year_high.empty:
            ytd_high = float(in_year_high.max())
    if ytd_high is None:
        ytd_high = float(in_year.max())
    if _is_finite(ytd_high) and ytd_high > 0:
        ytd_high_drawdown_pct = (float(latest_close) / ytd_high - 1.0) * 100.0

    return {
        "ytd_pct": (float(latest_close) / float(start_close) - 1.0) * 100.0,
        "daily_pct": daily_pct,
        "ytd_high_drawdown_pct": ytd_high_drawdown_pct,
    ...
    for ticker in tickers:
        if ticker in ticker_results:
            rows.append({
                ...
                "ytd_high_drawdown_pct": ticker_results[ticker]["ytd_high_drawdown_pct"],
                "latest_close": ticker_results[ticker]["latest_close"],
                "ytd_high": ticker_results[ticker]["ytd_high"],
