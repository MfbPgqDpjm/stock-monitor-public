...
def _momentum_top_rs_line(buy_signal: Dict) -> Optional[str]:
    """可买候选中 RS120 最高一行；无 eligible 则 None。"""
    top = top_eligible_rs_stock(buy_signal.get("scanned_stocks") or [])
    if not top:
        return None
    return f"RS120最高：{top['ticker']} {format_rs_pct(top['rs120'])}"
...
def _append_momentum_buy_signal_lines(
    ...
    """写入买入信号段；有 eligible 最高 RS120 时附带标注（与 UI 一致）。"""
    top = top_eligible_rs_stock(buy_signal.get("scanned_stocks") or [])
    ...
    if selected:
        ...
        for data in selected:
            ...
            rs120 = data.get("rs120")
            latest_price = data.get("latest_price")
            rs_str = format_rs_pct(rs120) if isinstance(rs120, (int, float)) else str(rs120)
            price_str = f"${latest_price:.2f}" if isinstance(latest_price, (int, float)) else str(latest_price)
            suffix = "（RS120最高）" if top and top.get("ticker") == ticker else ""
            body_lines.append(f"🟢 买入信号：{ticker} | RS120：{rs_str} | {price_str}{suffix}")
...
def notify_momentum_buy(bark_key: str, buy_signal: Dict, scan_time: Optional[datetime] = None) -> bool:
    ...
    if status == "有买入信号":
        ...
        price_str = f"${latest_price:.2f}" if isinstance(latest_price, (int, float)) else str(latest_price)
        suffix = "（RS120最高）" if top and top.get("ticker") == ticker else ""
        body = f"{ticker} | RS120：{rs_str} | {price_str}{suffix}"
        top_line = _momentum_top_rs_line(buy_signal)
...
def notify_momentum_combined(
    ...
    if sell_positions:
        title = f"🔴 动量系统 ({len(sell_positions)} 待卖出)"
